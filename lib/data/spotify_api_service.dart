...
class SpotifyApiService {
  ...
  Future<List<ArtistObject>> getTopArtists(TimeInterval timeInterval) async {
    ...
    final String url = "https://api.spotify.com/v1/me/top/artists?time_range=${timeInterval.apiName}";
    return http.get(Uri.parse(url), headers: {
      ...
      ErrorHandler.logger
          .i("Url: ${response.request?.url.toString()} \nStatus code: ${response.statusCode}\nbody: ${response.body}");
      if (response.statusCode != 200) {
        throw RestApiException(message: response.body, statusCode: response.statusCode);
      }
  ...
  Future<List<TrackObject>> getTopTracks(TimeInterval timeInterval) async {
    final String? accessToken = await _tokenRepository.getToken();
    final String url = "https://api.spotify.com/v1/me/top/tracks?time_range=${timeInterval.apiName}";
    return http.get(Uri.parse(url), headers: {
      'Content-Type': 'application/json',
      'Accept': 'application/json',
      'Authorization': 'Bearer $accessToken',
