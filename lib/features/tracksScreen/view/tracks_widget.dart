            ...
            body: Column(
              ...
              children: [
                Padding(
                  ...
                  child: Text(LocaleKeys.yourTopTracks.tr(), style: Theme.of(context).textTheme.titleLarge),
                ),
                Flex(
                  ...
                  children: [
                    SecondButtonWidget(
                        ...
                        onPressed: () => context.read<TracksCubit>().changeTimeInterval(TimeInterval.last4Weeks),
                        isActive: state.timeInterval == TimeInterval.last4Weeks),
                    SecondButtonWidget(
                        text: LocaleKeys.last6Months.tr(),
                        onPressed: () => context.read<TracksCubit>().changeTimeInterval(TimeInterval.last6Months),
                        isActive: state.timeInterval == TimeInterval.last6Months),
                    SecondButtonWidget(
                        text: LocaleKeys.allTime.tr(),
                        onPressed: () => context.read<TracksCubit>().changeTimeInterval(TimeInterval.allTime),
                  ...
                  child: BlocBuilder<TracksCubit, TracksState>(
                    builder: (context, state) {
                      if (state is TracksLoadedState) {
                        return GridView.count(crossAxisCount: 2, children: [...listOfTracks(state, context)]);
                      } else {
                        return const Center(
